...
AuthRoutes.post(
  ...
  authLimiter, async (req: Request<{}, {}, ReqBody_Authenticate>, res: Response) => {
    ...
    if (!isEthereumAddress && !isStarknetAddress) {
      return res.status(400).json({ success: false, message: 'Invalid wallet address format or type' });
    }
    ...
    if (isEthereumAddress) {
      if (!message) {
        return res.status(400).json({ success: false, message: 'Message is required for Ethereum verification' });
      }
