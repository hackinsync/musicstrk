...
use contract_::audition::types::season_and_audition::{
    Genre, VotingConfig, VoteType, ArtistScore, UnifiedVote,
};
use contract_::events::{
    UnifiedVoteCast, VotingConfigSet, ArtistScoreUpdated, CelebrityJudgeSet,
};
use snforge_std::{
...
use crate::test_utils::{
    OWNER, USER, NON_OWNER, deploy_contracts_with_staking, default_contract_create_season,
    default_contract_create_audition,
};
...
fn setup_audition_with_participants() -> (ISeasonAndAuditionDispatcher, IStakeToVoteDispatcher, u256) {
    ...
    staking_dispatcher.set_staking_config(audition_id, 1000, stake_token_address, 86400); // 1 day withdrawal delay
...
fn test_unified_voting_integration_judge_and_staker_votes() {
    let (audition_dispatcher, _staking_dispatcher, audition_id) = setup_audition_with_participants();
    ...
    let celebrity_vote = audition_dispatcher.get_unified_vote(audition_id, artist_id_2, CELEBRITY_JUDGE());
    ...
    spy.assert_emitted(@array![
        (audition_dispatcher.contract_address, UnifiedVoteCast {
            audition_id,
            artist_id,
            voter: JUDGE1(),
            weight: 1000,
...
fn test_automatic_role_detection_accuracy() {
    ...
    start_cheat_caller_address(audition_dispatcher.contract_address, CELEBRITY_JUDGE());
    audition_dispatcher.cast_vote(audition_id, artist_id, IPFS_HASH_2());
    let celebrity_vote = audition_dispatcher.get_unified_vote(audition_id, artist_id, CELEBRITY_JUDGE());
...
fn test_edge_case_nonexistent_audition() {
    let (audition_dispatcher, _staking_dispatcher, _audition_id) = setup_audition_with_participants();
